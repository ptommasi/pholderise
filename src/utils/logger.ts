...
function longerTime(now: Date) {

  const day   = addZ (now.getDate());
  const month = addZ (now.getMonth() + 1);

  const hour    = addZ ( now.getHours()        );
  const minutes = addZ ( now.getMinutes()      );
  const seconds = addZ ( now.getSeconds()      );

  return `${day}/${month} at ${hour}:${minutes}:${seconds}`;
...
function safeStringify(obj: any) {
  try {
    ...
    console.error("The object is impossible to parse therefore won't be part of the log: \n", obj, "\nError: ", err);
    return `<Cannot parse passed object: ${err.name}: ${err.message}>`;
  }
}

function storeLog(groupId: string | null, time: number, level: LogLevel, rawMessages: any[]) {
  const niceTime = longerTime(new Date(time));
  const messages = rawMessages.map(rw => typeof rw === "string" ? rw : safeStringify(rw));
  pastLogs.push({ groupId, time, level, messages, niceTime });
...
export const logger = {
  ...
  error: (...arg: any[]) => {
    const now = new Date();
    console.error(chalk.bold.red(time(now), 'ERROR'), ...arg);
...
export const loggerWithId = {
  ...
  error: (id: string, ...arg: any[]) => {
    const now = new Date();
    console.error(chalk.bold.red(time(now), 'ERROR', `[${id}]`), ...arg);
